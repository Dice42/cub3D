...
{
	if (cos(degToRad(cube->player.rays.angle)) > 0.001)
	{
		cube->player.rays.start_rx = (((int(cube->player.x0) / 64) * 64) + 64);
		cube->player.rays.start_ry = (cube->player.x0 - cube->player.rays.start_rx) * tan(degToRad(cube->player.rays.angle) + );
	}
}
