...
/*   Updated: 2024/08/02 19:30:04 by mohammoh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_textured_vertical_line(t_cub3d *cube, int x, int start_y, int end_y, t_img_data *texture, float texture_x)
{
	(void)texture_x;
	int y;
	int texture_height = texture->line_length / (texture->bits_per_pixel / 8);
	...
	while (y <= end_y)
	{
		int texture_y = (float)((y - start_y) * texture_height) / (end_y - start_y);
		...
		// printf("y: %d, color: %d\n", y, color);
		my_mlx_pixel_put(&cube->data.img, x, y,0x800000);
		y++;
	}
}
...
void minimap_rays(t_cub3d *cube, int x)
{
	...
	float line_height = (8 * HEIGHT) / distance;
...
void cast_rays_from_player(t_cub3d *cube)
{
	...
	cube->player.rays.angle = (cube->player.transform.angle - (30.0 * RAD));  // Start angle
